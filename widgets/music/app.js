...
playlist.addEventListener("change", function () {
  // use regex to get playlist id
  const playlistId = this.value.match(/list=([^&]+)/)[1];
  customPlaylist.custom.src = playlistId;
  playlists.custom = customPlaylist.custom;
  let option = document.createElement("option");
  option.value = playlists.custom.src;
  option.text = playlists.custom.name;
  selector.add(option);
  // auto select custom playlist
  selector.value = playlists.custom.src;
...
async function onPlayerStateChange(event) {
  ...
  if (event.data == YT.PlayerState.PLAYING) {
    ...
    if (apiKey) {
      ...
      //If no API Key is provided, use the deprecated function through the iFrame API
...
audio.addEventListener("input", function () {
  if (audio.value < 5) {
    //Bad way of doing it, but it works :) , doing audio.value < 1 doesn't work.
    player.mute();
